...
public class poKayakFlightsSearch1 extends TestBase{

	Logger log = Logger.getLogger(getClass().getSimpleName());

	public poKayakFlightsSearch1(WebDriver driver) {
		this.driver = driver;
...
//			oBroUti.ufClick(isDisplay_origin);
	...
	@FindBy(xpath="//div[contains(@id,'cal-202010')]//div[@class='month ']//*[contains(text(),'October 2020')]")
	...
	@FindBy(xpath="//div[contains(@id,'cal-202010']//div[@class='month ']//div[contains(text(),'October 2020')]")
...
//      oBroUti.waitForPageElementToVisible(driver, datewidget_weeks, 60);
    ...
    /*  for (WebElement datecell1 : datewidget_columns) {
           // Select Date
           if (datecell1.getAttribute("aria-label").contai
           ns(dateArray1[1])) {
	...
	{
		oBroUti.waitForPageElementToVisible(driver, closePopup, 80);
		closePopup.click();	
	}
	...
	public boolean searchForMultipleFlightsResultsInfo() throws Throwable{
		boolean bRes_Flag=false;
		try {
	...
	@FindBy(xpath="//button[@class='tab overview-tab Button-No-Standard-Style'][contains(text(),'Details ')]")
	public WebElement tab_Details;
	@FindBy(xpath="//div[contains(@id,'detailsContent')]//section[@class='details-section covidPolicySection']//p[contains(text(),'Airline policy updates')]")
...
//			oBroUti.waitForPageElementToVisible(driver, label_airlinepolicyupdates, 90);	
	...
	@FindBy(xpath="//div[contains(@id,'airports-section-airports-legs')]//div")
	public WebElement label_origindestination;
	@FindBy(xpath="(//div[contains(@class,'col-7-12 col-segment-heading')])[1]")
	public WebElement origin_destination;
	@FindBy(xpath="//*[contains(@id,'info-leg-0-origin-airport') and (@class,'bottom-airport')]//span")
	public WebElement title_originairport;
	@FindBy(xpath="//*[contains(@id,'info-leg-0-destination-airport') and (@class,'bottom-airport')]//span")
	public WebElement title_destinationairport;
	
	public boolean checkoriginanddestinationcitiesmatchesornot() throws Exception{
		boolean bRes_Flag=false;
		oBroUti.ScrollToView(driver, origin_destination);
		oBroUti.waitForPageElementToVisible(driver, origin_destination, 60);	
		oBroUti.ufGetText(origin_destination);
	//	oBroUti.ScrollToView(driver, label_destination);
	//oBroUti.waitForPageElementToVisible(driver, label_destination, 60);	
		oBroUti.ufGetText(origin_destination);
		log.info("origin and destination cities matches: "+oBroUti.ufGetText(origin_destination));		
			bRes_Flag=true;
		return bRes_Flag;
		}
	...
	@FindBy(xpath="(//div[@class='left-column segment-dates'])[1]")
	public WebElement date_departure;
	@FindBy(xpath="(//div[@class='left-column segment-dates'])[2]")
	public WebElement date_return;
	...
	public boolean checkdepartureandreturndatesmatchesornot() throws Exception{
		boolean bRes_Flag=false;
		oBroUti.waitForPageElementToVisible(driver, date_departure, 60);	
		oBroUti.ufGetText(date_departure);
		oBroUti.ufGetText(date_return);
		log.info("departure and return dates matches: "+oBroUti.ufGetText(date_departure));		
			bRes_Flag=true;
		return bRes_Flag;
		}
